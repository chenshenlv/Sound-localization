...
try:
    ...
    new_y_axis=hp_fun.y_hedge_axis(rotation_angle_z)
    hedge_obj_vector=hedge_iner_posi-np.array(obj_position_list)
    
    azimuths_body=math.floor(hp_fun.calculateAzimuths(new_y_axis,hedge_obj_vector.flatten()))
...
#                        hedge_obj_vector=np.array([1,0,0])-np.asarray(obj_position_list) #sudo test code 
                        azimuths_body=hp_fun.calculateAzimuths(new_y_axis,hedge_obj_vector.flatten())
