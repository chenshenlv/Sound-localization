...
hp_fun=help_fun()
obj1 = objects(1.9, -3.5, 0, './SoundTrack/group1.wav')
obj2 = objects(5, -3.5, 0, './SoundTrack/group2.wav')
obj3 = objects(7.9, -3.5, 0, './SoundTrack/group3.wav')
obj4 = objects(11.3, -3.07, 0, './SoundTrack/group4.wav')
obj5 = objects(17.3, -4.87, 0, './SoundTrack/group5.wav')
obj6 = objects(20.5, -5.01, 0, './SoundTrack/group6.wav')
obj_position_list=[obj1.position, obj2.position, obj3.position, obj4.position, obj5.position, obj6.position]
objsound=[obj1.soundfile, obj2.soundfile, obj3.soundfile, obj4.soundfile, obj5.soundfile, obj6.soundfile]
HRTF_data = loadmat('./HRTF/58_Propcessed_HRTF.mat')
azimuths =HRTF_data['azimuths'].flatten()
azimuths = azimuths.tolist()
previous_playlist = collections.deque([0]*1, maxlen=1)

padding_len = len(np.squeeze(HRTF_data['hrir_l'][0, :]))-1 # 200*1

#event = threading.Event() 
ip_Gui='192.168.1.6'  #ip address for GUI
port_Gui = 8000
socket = client()
try:
    socket.connect(ip_Gui,port_Gui)
except OSError:
    socket.close()
...
#        sorted_dis_index = [1,0,2]
        ...
        difference_list = list(set(sorted_dis_index[0:1])-set(previous_playlist))
        remove_thread_list = list(set(previous_playlist)-set(sorted_dis_index[0:1]))
...
#                            previous_playlist.remove(remove_thread_no)
                            ...
                            print('previous_playlist /ValueError',previous_playlist)
                        break
#                        time.sleep(0.2)
