...
#        delay_low = HRTF_data['ITD'][aIndex_low]
#        delay_up = HRTF_data['ITD'][aIndex_up]

#        left_low = np.squeeze(HRTF_data['hrir_l'][aIndex_low, 8, :])  # 200*1
#        right_low = np.squeeze(HRTF_data['hrir_r'][aIndex_low, 8, :])  # 200*1
#        left_up = np.squeeze(HRTF_data['hrir_l'][aIndex_up, 8, :])  # 200*1
#        right_up = np.squeeze(HRTF_data['hrir_r'][aIndex_up, 8, :])  # 200*1
#        delay_low = HRTF_data['ITD'][aIndex_low, 8]
#        delay_up = HRTF_data['ITD'][aIndex_up, 8]
...
#        print("lamda:", lamda)
        left_inter=(1-lamda)*left_low+lamda*left_up    # interpolation value
        right_inter=(1-lamda)*right_low+lamda*right_up
#        delay=(1-lamda)*delay_low+lamda*delay_up
        
        return left_inter, right_inter

#q1=np.array([2,1,1,0])
#v1=np.array([1,0,0])
#vb=quaternionRotate(q1,v1)
#angle=quaternionToEulerianAngle(q1)
#azimuths=calculateAzimuths(vb)
#obj=[[1,0,0],
#     [2,0,0],
#     [3,0,0],
#     [1,1,1],
...
#
#left=HRTF_Interpolation(azimuths,azimuths_body,HRTF_data)[0]